...
echo "Features: Complete SM_121 support, GB10 MoE optimization (+65.7%)"
...
echo "  vLLM: main branch (latest) with SM_121 patches"
echo "  PyTorch: stable (CUDA 13.0)"
echo "  FlashInfer: latest pre-release"
echo "  XGrammar: latest stable"
echo "  Backend: TORCH (FP8 linear) + TRITON (FP8 MoE)"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  ...
  ssh "${REMOTE_USER}@${REMOTE_NODE}" \
    "cd /tmp/dgx-vllm-build && docker build -t ${IMAGE_NAME}:${IMAGE_TAG} -t ${IMAGE_NAME}:v${IMAGE_VERSION} -t ${IMAGE_NAME}:sm121 ." \
...
echo "  2. Verify SM_121 backend selection (check logs for '[vLLM SM_121]' messages)"
echo "  3. Benchmark with GB10 hardware (expect ~39 tok/s with Qwen3-Next-80B)"
